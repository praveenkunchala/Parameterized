...
break;
		...
		case "Natwest Premier":
			...
			/*
			 * try { boolean B = Baseclass.WaitElementVisible(driver,
			 * By.xpath(elementProperties.getProperty("Business_Type"))) .isDisplayed();
			 * Baseclass.explicitlyWait(Baseclass.timeout); if (B) {
			 * test.log(LogStatus.PASS, "Business type drop down  is displayed"); } else {
			 * test.log(LogStatus.FAIL, "Business type drop down  is not displayed"); }
			 * 
			 * } catch (StaleElementReferenceException | ElementNotInteractableException |
			 * TimeoutException e) { test.log(LogStatus.FAIL,
			 * "Business Type drop down not displayed"); e.printStackTrace(); }
			 */
			/*
			 * try { Baseclass.scrollDownFull(); boolean BT = Baseclass
			 * .WaitElementVisible(driver,
			 * By.xpath(elementProperties.getProperty("Business_turnover"))) .isDisplayed();
			 * Baseclass.explicitlyWait(Baseclass.timeout); if (BT) {
			 * test.log(LogStatus.PASS, "Business turn over drop down  is displayed"); }
			 * else { test.log(LogStatus.FAIL,
			 * "Business turn over drop down  is not displayed"); }
			 * 
			 * } catch (StaleElementReferenceException | ElementNotInteractableException |
			 * TimeoutException e) { test.log(LogStatus.FAIL,
			 * "Business turn over drop down  not displayed"); e.printStackTrace(); }
			 */
			try {
				boolean BL = Baseclass
						.WaitElementVisible(driver, By.xpath(elementProperties.getProperty("location")))
						.isDisplayed();
				Baseclass.explicitlyWait(Baseclass.timeout);
				if (BL) {
					test.log(LogStatus.PASS, "Business location drop down  is displayed");
			...
			try {
			boolean b1 = Baseclass.WaitElementVisible(driver,
					By.xpath(elementProperties.getProperty("premier_age"))).isDisplayed();
			Baseclass.explicitlyWait(Baseclass.timeout);
			if (b1) {
				test.log(LogStatus.PASS, "Age drop down  is displayed");
			} else {
				test.log(LogStatus.FAIL, "Age drop down  is not displayed");
			}

		} catch (StaleElementReferenceException | ElementNotInteractableException | TimeoutException e) {
			test.log(LogStatus.FAIL, "Age drop down  not displayed");
			e.printStackTrace();
		}
